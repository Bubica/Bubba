"""
Tweaked version of ebay.finding module to support a multi value item filter.
"""
...
def findItemsByKeywords(
    ...
    if itemFilter:
        for item in itemFilter: #single dict
    ...
    if aspectFilter:
        for item in aspectFilter: #single dict
...
def findItemsAdvanced(
        ...
        descriptionSearch=False, encoding="JSON"):
    ...
    #domainSearch is boolean
    descriptionSearch_elem = etree.SubElement(root, "descriptionSearch")
    descriptionSearch_elem.text = str(descriptionSearch).lower()
