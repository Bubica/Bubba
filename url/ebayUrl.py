...
def parse(url, name=None):
    ...
    sinstance = search.SearchRequest(id_= url)
    ...
    if '_sacat' in d_url:
        ...
        pattern = "^http://www.ebay.co.uk/"+"(.*?)"+"\?"+"(.*?)"

        m = re.split(pattern, url)
    ...
    if 'Color' in d_url:                   sinstance['Color']       = _format_multi_param(d_url['Colour'])
